...
function PlayState:update(dt)
    ...
    if self.canInput then
        ...
        if love.mouse.isDown(1) then
            ...
            propY = y - self.board.y
            propX = x - self.board.x
            tempY = ((propY + 16) / 32) + 0.5
            tempX = ((propX + 16) / 32) + 0.5

            if (tempY - 1) > 7 then
                self.boardHighlightY = 7
                self.boardHighlightX = math.floor(tempX) - 1
            elseif (tempY - 1) < 1 then
                self.boardHighlightY = 0
                self.boardHighlightX = math.floor(tempX) - 1
            elseif (tempX - 1) > 7 then
                self.boardHighlightX = 7
                self.boardHighlightY = math.floor(tempY) - 1
            elseif (tempX - 1) < 1 then
                self.boardHighlightX = 0
                self.boardHighlightY = math.floor(tempY) - 1
            else
               tempY = ((propY + 16) / 32) + 0.5
        ...
        self.boardHighlightX = math.max(0,self.boardHighlightX)
        self.boardHighlightY = math.min(7,self.boardHighlightY)
        self.boardHighlightY = math.max(0,self.boardHighlightY)
        ...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
            ...
            -- clearly it has wrong gridx coordinates as that's what ir 
