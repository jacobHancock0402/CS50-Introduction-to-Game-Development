...
function Level:update(dt)
    ...
    if self.launchMarker.launched then
        if love.keyboard.wasPressed('space') and not self.collided and not self.reproduce then
            love.audio.play(gSounds['powerup'])
            local x, y = self.launchMarker.alien[1].body:getPosition()
            self.reproduce = true
            local speedx, speedy = self.launchMarker.alien[1].body:getLinearVelocity()
            y2 = math.min(VIRTUAL_HEIGHT - 35, y + 40)
            if y2 < VIRTUAL_HEIGHT - 35 then
                Alien2 = Alien(self.world, 'round', x, y2, 'Player')
                table.insert(self.launchMarker.alien, Alien2 )
            end
            Alien3 = Alien(self.world, 'round', x, y - 40, 'Player')
            table.insert(self.launchMarker.alien, Alien3 )
            for k, ali in pairs(self.launchMarker.alien) do
                ali.spawned = true
                if k == 2 then
                    ali.body:setLinearVelocity((speedx) * 0.75, (speedy) * 0.75)
                elseif k == 3 then 
                    ali.body:setLinearVelocity((speedx) * 1.25, (speedy) * 1.25)
                end
    
                    -- make the alien pretty bouncy
                ali.fixture:setRestitution(0.4)
                ali.body:setAngularDamping(1)
            end
        end
        for k, alien in pairs(self.launchMarker.alien) do
            if alien.spawned then
                local xPos, yPos = alien.body:getPosition()
                local xVel, yVel = alien.body:getLinearVelocity()
                if yPos > VIRTUAL_HEIGHT - 35 then
                    alien.body:destroy()
                ...
                if xPos < 0 or (math.abs(xVel) + math.abs(yVel) < 1.5) then
                    alien.body:destroy()
                    table.remove(self.launchMarker.alien, k)
                    if next(self.launchMarker.alien) == nil then
                        self.launchMarker = AlienLaunchMarker(self.world)
                    end
                end
