...
-- @int cnfg.nu***REMOVED***ields indices of columns known to be numbers
...
function data.read(file,cnfg)
    ...
    local conversion
    local nu***REMOVED***ields = {}
    local function append_conversion (idx,conv)
        conversion = conversion or {}
        append(nu***REMOVED***ields,idx)
        append(conversion,conv)
    end
    if cnfg.nu***REMOVED***ields then
        for _,n in ipairs(cnfg.nu***REMOVED***ields) do append_conversion(n,try_number) end
    end
    ...
    if cnfg.fieldnames then
        ...
        -- of the first data row. These can also be specified explicitly by @nu***REMOVED***ields.
        if not cnfg.no_convert then
            local fields = split(line,D.delim,csv,nfields)
            for i = 1,#fields do
                if not find(nu***REMOVED***ields,i) and try_number(fields[i]) then
    ...
    while line do
        if not line:find ('^%s*$') then -- [blank lines] ignore them!
            ...
            if conversion then -- there were field conversions...
                for k = 1,#nu***REMOVED***ields do
                    local i,conv = nu***REMOVED***ields[k],conversion[k]
...
local function process_select (data,parms)
    ...
    local nu***REMOVED***ields = fields:find '%$'  or #data.fieldnames == 0
    if fields:find '^%s*%*%s*' then
        if not nu***REMOVED***ields then
