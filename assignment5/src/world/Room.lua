...
function Room:update(dt)
    ...
    for i = #self.entities, 1, -1 do
        ...
        if entity.health <= 0 and not entity.dead then
            entity.dead = true
            -- maybe def is wrong
            if entity.heal == 1 then
                heart = GameObject(GAME_OBJECT_DEFS['heart'])
                heart.x, heart.y = entity.x, entity.y
    ...
    for k, object in pairs(self.objects) do
        ...
        if object.projectile then
            if not self.projectile then
                epot = Projectile(self.player.direction, object,object.x,object.y, self.entities, self.player)
                table.insert(self.projectiles, epot)
            end
            counter = 1
        end
        ...
        if self.player:collides(object) then
            ...
            elseif object.texture == 'pots' and not object.projectile and object.state == 'solid' then
                if self.player.carrying == false then
                    love.audio.play(gSounds['pickup'])
                end
...
function Room:render()
    ...
    for k, entity in pairs(self.entities) do
        if not entity.dead then entity:render(self.adjacentOffsetX, self.adjacentOffsetY)
        end
    end

    for k, projectile in pairs(self.projectiles) do 
    if projectile.object.state == 'broken' then
